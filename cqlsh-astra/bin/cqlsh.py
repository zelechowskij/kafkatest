...
CASSANDRA_CQL_HTML_FALLBACK = 'https://docs.datastax.com/en/dse/6.8/cql/index.html'
...
try:
    ...
    sys.exit("\nDataStax Python Driver not installed, or not on PYTHONPATH.\n"
...
from cassandra.metadata import (
    ColumnMetadata, KeyspaceMetadata,
...
from cqlshlib.displaying import (ANSI_RESET, BLUE, COLUMN_NAME_COLORS, CYAN,
                                 GREEN, RED, WHITE, FormattedValue, colorme)
...
parser = optparse.OptionParser(description=description, epilog=epilog,
                               usage="Usage: cqlsh [options] [host [port]]",
...
parser.add_option('-b', '--secure-connect-bundle', help="Connect using secure connect bundle. If this option is specified host, port settings are ignored")
...
parser.add_option("--dse-protocol-version", type="int", default=None,
                  help='Specify a specific DSE protocol version; otherwise the client will default and downgrade as necessary. Mutually exclusive with --protocol-version.')
parser.add_option("--protocol-version", type="int", default=None,
                  help='Specify a specific protocol version; otherwise the client will default and downgrade as necessary. Mutually exclusive with --dse-protocol-version.')
...
class Shell(cmd.Cmd):
    ...
    def perform_simple_statement(self, statement):
        ...
        if self.timing_enabled:
            request_rtt = time() - request_start
        ...
        if statement.query_string[:6].lower() == 'select':
            self.print_result(result, self.parse_for_select_meta(statement.query_string), request_rtt)
        elif statement.query_string.lower().startswith("list"):
            self.print_result(result, None, request_rtt)
        elif statement.query_string.lower().startswith("run") or statement.query_string.lower().startswith("restore"):
            # This statements aren't tied to a particular table, therefor we just rely on the result set metadata
            self.print_static_result(result, None, request_rtt)
        elif result:
            # CAS INSERT/UPDATE
            self.writeresult("")
            self.print_static_result(result, self.parse_for_update_meta(statement.query_string), request_rtt)
        self.flush_output()
        return True, future

    def print_result(self, result, table_meta, request_rtt=None):
        ...
        if result.has_more_pages and self.tty:
            ...
            while True:
                if result.current_rows:
                    ...
                    self.print_static_result(result, table_meta, request_rtt)
    ...
    def print_static_result(self, result, table_meta, request_rtt=None):
        ...
        if not result.current_rows:
            ...
            if self.timing_enabled and request_rtt is not None:
                self.writeresult("%7.6f seconds elapsed" % request_rtt, color=GREEN)
            return
    ...
    def do_describe(self, parsed):
        ...
        """
        try:
            # refresh the schema before describing anything to make sure we don't show outdated info
            self.conn.refresh_schema_metadata(max_schema_agreement_wait=-1)
        except Exception:
            # don't let errors bubble up to the user in case refreshing schema failed
            pass
    ...
    def do_source(self, parsed):
        ...
        subshell = Shell(self.hostname, self.port, color=self.color,
                         ...
                         consistency_level=self.consistency_level, serial_consistency_level=self.serial_consistency_level,
    ...
    def do_timing(self, parsed):
        ...
        TIMING [cqlsh]
          ...
          Enables or disables simple request round-trip timing, as measured on the current cqlsh session.
...
def read_options(cmdlineargs, environment):
    ...
    optvalues.dse_protocol_version = option_with_default(configs.getint, 'protocol', 'dse_version', None)
    ...
    try:
        ...
        if cl in [cassandra.ConsistencyLevel.SERIAL, cassandra.ConsistencyLevel.LOCAL_SERIAL]:
            raise KeyError
    except KeyError:
        cl = cassandra.ConsistencyLevel.name_to_value[default_cl]
        sys.stderr.write(
            '"{}" is not a valid consistency level, falling back to {}\n'
            .format(options.consistency_level, default_cl))
    options.consistency_level = cl

    try:
        cl = cassandra.ConsistencyLevel.name_to_value[options.serial_consistency_level.upper()]
        if cl not in [cassandra.ConsistencyLevel.SERIAL, cassandra.ConsistencyLevel.LOCAL_SERIAL]:
            raise KeyError
    except KeyError:
        cl = cassandra.ConsistencyLevel.SERIAL
        sys.stderr.write(
            '"{}" is not a valid serial consistency level, falling back to {}'
    ...
    if options.protocol_version is not None and options.dse_protocol_version is not None:
        parser.error("--protocol-version and --dse-protocol-version are mutually exclusive (version or dse_version may also be set in the cqlshrc [protocol] section).")
...
def main(options, hostname, port):
    ...
    if options.debug:
        ...
        sys.stderr.write("Using Consistency Level: %s, connect timeout: %s seconds\n"
            % (cassandra.ConsistencyLevel.value_to_name[options.consistency_level], options.connect_timeout))
    ...
    if options.dse_protocol_version is not None:
        options.protocol_version = options.dse_protocol_version | 0x40
